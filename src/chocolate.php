...
return $in;	
	...
	//To convert  text  to image using a particular background
		

	Public Function TextToImage($text,$ImageName,$bg,$processFolder){			
...
$text = html_entity_decode($text)
	...
	if (!file_exists($ImageName)){		
		...
		//To restrict text size from being too big when texts are short in count
...
try {
    //
    // WARNING: This will create a lot of images in the processing folder
    //

    $imagewidth = 600;
 
 // Best fit
    $this->imgconv->load($bg)->resize(700, 500)->save($processFolder.'/'.$uniqueHash.'1-fit.jpg');
   ...
   // Text
   $this->imgconv->load($processFolder.'/'.$uniqueHash.'overlaid.jpg')->imagettftextboxopt($size,$angle,$left,$top,'#fff',$sbp.'/assets/fonts/Hallo sans black.otf',$text,$sbp.'/'.$ImageName,$imagewidth,500,$line_height);

   
   //To destroy and delete all processing file
   unlink($processFolder.'/'.$uniqueHash.'1-fit.jpg');
   unlink($processFolder.'/'.$uniqueHash.'overlaid.jpg');
